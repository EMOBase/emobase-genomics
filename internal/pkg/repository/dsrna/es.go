...
func (r *ElasticSearchRepository) SaveMany(ctx context.Context, indexName string, docs []entity.DsRNA) error {
	for i := 0; i < len(docs); i += r.batchSize {
		end := min(i+r.batchSize, len(docs))
		if err := r.bulkIndex(ctx, indexName, docs[i:end]); err != nil {
			return err
		}
	}
	return nil
...
func (r *ElasticSearchRepository) bulkIndex(ctx context.Context, indexName string, docs []entity.DsRNA) error {
	var buf bytes.Buffer
	enc := json.NewEncoder(&buf)

	for _, doc := range docs {
		meta := map[string]map[string]string{
			"index": {"_id": doc.GetID(), "_index": indexName},
		}
